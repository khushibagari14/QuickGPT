  ...
  className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b
    ...
    ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      ...
      <div className=' flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
        ...
        <input 
          onChange={(e)=>setSearch(e.target.value)}   // e=event
          value={search} 
          type="text"
...
<div className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md'>
  ...
  <label className='relative inline-flex cursor-pointer'>
    ...
    rounded-full transition-transform peer-checked:translate-x-4'></span>
  </label>
</div>
...
{/* Close Button */}
<img onClick={()=> setIsMenuOpen(false)} src={assets.close_icon} className='absolute top-3 right-3
 w-5 h-5 cursor-pointer md:hidden invert dark:invert-0' alt="" />
